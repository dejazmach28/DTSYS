...
export default function NetworkInfo({ deviceId }: Props) {
  ...
  if (isLoading) {
    return <div className="text-sm text-gray-500">Loading network data...</div>
  }

  if (interfaces.length === 0) {
    return <div className="text-sm text-gray-500">No network data</div>
  }
      ...
      <table className="min-w-full text-left text-sm">
        ...
        <tbody>
          {interfaces.map((iface) => (
            <tr key={iface.id} className="border-b border-gray-900 text-gray-300">
              <td className="px-3 py-3 font-medium text-gray-100">{iface.interface_name}</td>
